...
from utils_hand4whole.human_models import smpl_x, smpl
from utils_hand4whole.transforms import cam2pixel, transform_joint_to_other_db
...
def process_human_model_output(human_model_param, cam_param, do_flip, img_shape, img2bb_trans, rot, human_model_type):
    ...
    if human_model_type == "smplx":
        ...
        joint_img[smpl_x.joint_part["body"], 2] = (joint_cam[smpl_x.joint_part["body"], 2].copy() / (cfg.body_3d_size / 2) + 1) / 2.0 * cfg.output_hm_shape[0]  # body depth discretize
        joint_img[smpl_x.joint_part["lhand"], 2] = (joint_cam[smpl_x.joint_part["lhand"], 2].copy() / (cfg.hand_3d_size / 2) + 1) / 2.0 * cfg.output_hm_shape[0]  # left hand depth discretize
        joint_img[smpl_x.joint_part["rhand"], 2] = (joint_cam[smpl_x.joint_part["rhand"], 2].copy() / (cfg.hand_3d_size / 2) + 1) / 2.0 * cfg.output_hm_shape[0]  # right hand depth discretize
        joint_img[smpl_x.joint_part["face"], 2] = (joint_cam[smpl_x.joint_part["face"], 2].copy() / (cfg.face_3d_size / 2) + 1) / 2.0 * cfg.output_hm_shape[0]  # face depth discretize
        ...
        joint_img[:, 2] = (joint_cam[:, 2].copy() / (cfg.body_3d_size / 2) + 1) / 2.0 * cfg.output_hm_shape[0]  # body depth discretize
        ...
        joint_img[:, 2] = (joint_cam[:, 2].copy() / (cfg.hand_3d_size / 2) + 1) / 2.0 * cfg.output_hm_shape[0]  # hand depth discretize
