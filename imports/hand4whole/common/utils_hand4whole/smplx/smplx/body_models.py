...
from .utils import Struct, to_np, to_tensor, Tensor, Array, SMPLOutput, SMPLHOutput, SMPLXOutput, MANOOutput, FLAMEOutput, find_joint_kin_chain
...
class SMPL(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        output = SMPLOutput(
            vertices=vertices if return_verts else None, global_orient=global_orient, body_pose=body_pose, joints=joints, betas=betas, full_pose=full_pose if return_full_pose else None
        )
...
class SMPLLayer(SMPL):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if global_orient is None:
            batch_size = 1
            global_orient = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, 1, 1).contiguous()
        else:
            batch_size = global_orient.shape[0]
        if body_pose is None:
            body_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, self.NUM_BODY_JOINTS, 1).contiguous()
        if betas is None:
            betas = torch.zeros([batch_size, self.num_betas], dtype=dtype, device=device)
        if transl is None:
            transl = torch.zeros([batch_size, 3], dtype=dtype, device=device)
        full_pose = torch.cat([global_orient.reshape(-1, 1, 3), body_pose.reshape(-1, self.NUM_BODY_JOINTS, 3)], dim=1)

        vertices, joints = lbs(betas, full_pose, self.v_template, self.shapedirs, self.posedirs, self.J_regressor, self.parents, self.lbs_weights, pose2rot=True)
...
class SMPLH(SMPL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SMPLH, self).__init__(
            model_path=model_path, data_struct=data_struct, batch_size=batch_size, vertex_ids=vertex_ids, gender=gender, use_compressed=use_compressed, dtype=dtype, ext=ext, **kwargs
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        vertices, joints = lbs(self.betas, full_pose, self.v_template, self.shapedirs, self.posedirs, self.J_regressor, self.parents, self.lbs_weights, pose2rot=pose2rot)
...
class SMPLHLayer(SMPLH):
    def __init__(self, *args, **kwargs) -> None:
        ...
        super(SMPLHLayer, self).__init__(
            create_global_orient=False, create_body_pose=False, create_left_hand_pose=False, create_right_hand_pose=False, create_betas=False, create_transl=False, *args, **kwargs
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if global_orient is None:
            ...
            batch_size = global_orient.shape[0]
        if body_pose is None:
            body_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, 21, -1).contiguous()
        if left_hand_pose is None:
            left_hand_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, 15, -1).contiguous()
        if right_hand_pose is None:
            right_hand_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, 15, -1).contiguous()
        if betas is None:
            betas = torch.zeros([batch_size, self.num_betas], dtype=dtype, device=device)
        if transl is None:
            transl = torch.zeros([batch_size, 3], dtype=dtype, device=device)
...
class SMPLX(SMPLH):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SMPLX, self).__init__(model_path=model_path, data_struct=data_struct, dtype=dtype, batch_size=batch_size, vertex_ids=VERTEX_IDS["smplx"], gender=gender, ext=ext, **kwargs)
    ...
    def forward(
        ...
        **kwargs,
        ...
        full_pose = torch.cat([global_orient, body_pose, jaw_pose, leye_pose, reye_pose, left_hand_pose, right_hand_pose], dim=1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if right_hand_pose is None:
            right_hand_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, 15, -1).contiguous()
        if jaw_pose is None:
            jaw_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, -1, -1).contiguous()
        if leye_pose is None:
            leye_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, -1, -1).contiguous()
        if reye_pose is None:
            reye_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, -1, -1).contiguous()
        if expression is None:
        ...
        vertices, joints = lbs(shape_components, full_pose, self.v_template, shapedirs, self.posedirs, self.J_regressor, self.parents, self.lbs_weights, pose2rot=True)
...
class MANO(SMPL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(MANO, self).__init__(model_path=model_path, data_struct=data_struct, batch_size=batch_size, vertex_ids=vertex_ids, use_compressed=use_compressed, dtype=dtype, ext=ext, **kwargs)
...
class MANOLayer(MANO):
    def __init__(self, *args, **kwargs) -> None:
        ...
        super(MANOLayer, self).__init__(create_global_orient=False, create_hand_pose=False, create_betas=False, create_transl=False, *args, **kwargs)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if global_orient is None:
            ...
            batch_size = global_orient.shape[0]
        if hand_pose is None:
            hand_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, 15, -1).contiguous()
        if betas is None:
            betas = torch.zeros([batch_size, self.num_betas], dtype=dtype, device=device)
        if transl is None:
            transl = torch.zeros([batch_size, 3], dtype=dtype, device=device)
...
class FLAME(SMPL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(FLAME, self).__init__(model_path=model_path, data_struct=data_struct, dtype=dtype, batch_size=batch_size, gender=gender, ext=ext, **kwargs)
...
class FLAMELayer(FLAME):
    def __init__(self, *args, **kwargs) -> None:
        ...
        super(FLAMELayer, self).__init__(
            create_betas=False, create_expression=False, create_global_orient=False, create_neck_pose=False, create_jaw_pose=False, create_leye_pose=False, create_reye_pose=False, *args, **kwargs
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if global_orient is None:
            ...
            batch_size = global_orient.shape[0]
        if neck_pose is None:
            neck_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, 1, -1).contiguous()
        if jaw_pose is None:
            jaw_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, -1, -1).contiguous()
        if leye_pose is None:
            leye_pose = torch.zeros(3, device=device, dtype=dtype).view(1, 1, 3).expand(batch_size, -1, -1).contiguous()
...
def build_layer(model_path: str, model_type: str = "smpl", **kwargs) -> Union[SMPLLayer, SMPLHLayer, SMPLXLayer, MANOLayer, FLAMELayer]:
