...
class Partitioner(torch.nn.Module):
    MERGE_BODY_PARTS = {
        "smpl": sorted(
            [
                ...
                13,
                14,  # left and right shoulder blade
                22,
                23,  # left and right hand into wrists
            ],
            reverse=True,
        ),
        "smplx": sorted(
    ...
    def check_bbox_penetrations(self, bb_min, bb_max, abs_transforms, EPS=1e-4):
        ...
        hposed_corners = torch.bmm(abs_transforms.view(K, 1, 4, 4).repeat(1, 8, 1, 1).view(K * 8, 4, 4), hcorners.view(K * 8, 4, 1)).view(K * 8, 4)

        # project posed corners to each other's space
        back_trans = torch.inverse(abs_transforms)

        proj_corners = torch.bmm(
            back_trans.view(K, 1, 4, 4).repeat(1, K * 8, 1, 1).view(K * K * 8, 4, 4), hposed_corners.view(1, K * 8, 4).repeat(K, 1, 1).view(K * K * 8, 4, 1)  # K*K*8,4,4  # K*K*8,4,1
        ).view(K, K, 8, 4)[
            ..., :3
        ]  # K, K, 8, 3
    ...
    def partition_body(self, parametric_body):
        ...
        tight_vert_selector = [torch.where(torch.from_numpy(max_part == valid_labels[i]))[0] for i in range(n_parts) if i not in self.merge_body_parts]
        ...
        for b_part in range(len(tight_vert_selector)):
            vert_sec = torch.full((max_elem,), dtype=tight_vert_selector[b_part].dtype, fill_value=tight_vert_selector[b_part][0].item())
    ...
    def _sample_mesh_points(posed_vert, face_tensor, n_samples):
        ...
        meshes = Meshes(  # (B*n_parts,V,3)
            verts=posed_vert.unsqueeze(1).expand(-1, n_parts, -1, -1).contiguous().view(B * n_parts, -1, 3), faces=face_tensor.unsqueeze(0).expand(B, -1, -1, -1).contiguous().view(B * n_parts, -1, 3)
        )
    ...
    def sample_mesh_points(self, posed_vert, n_samples):
        ...
        central_points = self._sample_mesh_points(posed_vert, self.tight_face_tensor, n_tight_samples)
        uniform_points = self._sample_mesh_points(posed_vert, self.extended_face_tensor, n_ext_samples)
    ...
    def compute_abs_transformations(self, full_pose, posed_joints):
        ...
        abs_trans = torch.cat([F.pad(transforms.reshape(-1, 3, 3), [0, 0, 0, 1]), F.pad(posed_joints[:, :mK].reshape(-1, 3, 1), [0, 0, 0, 1], value=1)], dim=2).reshape(B, mK, 4, 4)
...
class COAPBodyModel(torch.nn.Module):
    ...
    def _attach_impl_code(self, smpl_output=None):
        ...
        if self.impl_code is None:
            ...
            smpl_output.full_pose.shape != self.impl_code["full_pose"].shape
            or not torch.allclose(smpl_output.full_pose, self.impl_code["full_pose"])
            or not torch.allclose(smpl_output.joints, self.impl_code["joints"])
        ):
            self.encode_body(smpl_output)
    ...
    def _self_penetration_loss(self, smpl_output, n_points_uniform, at_least_n_samples):
        ...
        with torch.no_grad():
            ...
            collision_candidates = self.partitioner.sample_collision_candidates(bb_min, bb_max, abs_trans, collided_inds, n_points_uniform)
    ...
    def get_part_colors():
        return np.array(
            [
                ...
                [1.00000000e00, 4.98039216e-01, 3.58602037e-16],
                [1.00000000e00, 1.00000000e00, 2.00000000e-01],
                ...
                [1.00000000e00, 9.17647059e-01, 8.47058824e-01],
                ...
                [6.27450980e-01, 8.23529412e-02, 0.00000000e00],
                [1.00000000e00, 4.35294118e-01, 3.80392157e-01],
                [7.37254902e-01, -7.17204074e-16, 2.82352941e-01],
                [1.00000000e00, 9.09803922e-01, 5.52941176e-01],
                [1.00000000e00, 7.84313725e-02, 4.15686275e-01],
