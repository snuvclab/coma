...
from detectron2.data import detection_utils as detectron_utils  # utils
...
from utils.visualization.visualize_video import generate_video_from_imgs
            ...
            for i, t in enumerate(timesteps):
                ...
                if use_adaptive_mask:
                    ...
                    if self.adaptive_mask_model.use_visualizer:
                        ...
                        plt.savefig(f"{visualization_save_dir}/images/{i:05}.png", bbox_inches="tight", pad_inches=0)
                        plt.close("all")
...
class PointRendPredictor:
    def __init__(self, cat_id_to_focus=0, pointrend_thres=0.9, device="cuda", use_visualizer=False, merge_mode="merge"):
        super().__init__()
        ...
        self.cfg.merge_from_file(COCO_SEG_CONFIG_PTH)
        self.cfg.MODEL.WEIGHTS = COCO_SEG_WEIGHTS_PTH
...
SAM_MODEL_PTH_DICT = {
    "vit_h": "./imports/segment-anything/sam_vit_h_4b8939.pth",
    "vit_l": "./imports/segment-anything/sam_vit_h_4b8939.pth",
    "vit_b": "./imports/segment-anything/sam_vit_h_4b8939.pth",
}


class SAMHumanPredictor(PointRendPredictor):
    def __init__(
        self,
        sam_key="vit_h",
        device="cuda",
        merge_mode="merge",
        is_sam_multitask_output=False,
        *pointrend_args,
        **pointrend_kwargs,
    ):
        super().__init__(*pointrend_args, **pointrend_kwargs)

        # get segmentation model
        sam = sam_model_registry[sam_key](checkpoint=SAM_MODEL_PTH_DICT[sam_key])
        sam.cuda()
        self.sam_seg_model = SamPredictor(sam)
        self.is_sam_multitask_output = is_sam_multitask_output

    def __call__(self, image, debug=False):
        ## pointrend segmentation (1st step)
        # run pointrend segmentation
        outputs = self.pointrend_seg_model(image)
        instances = outputs["instances"]
        ...
        ## if human not predicted in first step, just pass
        if mask.sum() == 0:
            if self.use_visualizer:
                return mask.astype(np.uint8), self.vis_seg_on_img(image, mask)
            else:
                return mask.astype(np.uint8), None

        ## sam segmentation (2nd step)
        initial_human_bbox = seg2bbox(mask)
        self.sam_seg_model.set_image(image)
        masks, scores, logits = self.sam_seg_model.predict(
            box=initial_human_bbox,
            multimask_output=self.is_sam_multitask_output,
        )
        mask = self.merge_mask(masks, scores=scores)

        return {"asset_mask": None, "mask": mask.astype(np.uint8), "vis": self.vis_seg_on_img(image, mask) if self.use_visualizer else None}


class SAMHumanPredictorWithAssetExclusion(SAMHumanPredictor):
    def __init__(
        self,
        *args,
        **kwargs,
    ...
    def set_presumed_asset_mask(self, presumed_asset_mask: np.ndarray):
        self.presumed_asset_mask = presumed_asset_mask
        self.presumed_asset_bbox = seg2bbox(presumed_asset_mask)
    ...
    def __call__(self, image, debug=False):
        ...
        ## sam segmentation for asset, and exclude the asset mask (3rd step)
        asset_masks, asset_scores, _ = self.sam_seg_model.predict(
            box=self.presumed_asset_bbox,
            multimask_output=self.is_sam_multitask_output,
        )
        asset_mask = self.merge_mask(asset_masks, scores=asset_scores)
        mask = np.logical_and(mask, np.logical_not(asset_mask))

        return {"asset_mask": asset_mask.astype(np.uint8), "mask": mask.astype(np.uint8), "vis": self.vis_seg_on_img(image, mask) if self.use_visualizer else None}




class SAMHumanPredictorWithDefaultBboxAssetExclusion(SAMHumanPredictorWithAssetExclusion):
    def __init__(
        self,
        *args,
        **kwargs,
        ...
        super().__init__(*args, **kwargs)
        self.initial_human_bbox = None

    def set_initial_human_bbox(self, human_seg_np):
        self.initial_human_bbox = seg2bbox(human_seg_np)

    def reset_initial_human_bbox(self):
        self.initial_human_bbox = None

    def __call__(self, image, debug=False):
        ## if initial_human_bbox is not set, you need to first predict via pointrend
        ## if initial_human_bbox is set, skip this step
        if self.initial_human_bbox is None:
            ## pointrend segmentation (1st step)
            # run pointrend segmentation
            outputs = self.pointrend_seg_model(image)
            instances = outputs["instances"]
            ...
            if mask.sum() == 0:
                ...
                self.initial_human_bbox = seg2bbox(mask)

        ## regsiter image to sam
        self.sam_seg_model.set_image(image)
        ## sam segmentation (2nd step)
        masks, scores, logits = self.sam_seg_model.predict(
            box=self.initial_human_bbox,
            multimask_output=self.is_sam_multitask_output,
        )
        mask = self.merge_mask(masks, scores=scores)
...
class SAMHumanPredictorAccumulativeBboxAssetExclusion(SAMHumanPredictorWithDefaultBboxAssetExclusion):
    def __init__(
        self,
        *args,
        **kwargs,
    ...
    def __call__(self, image, debug=False):
        ## initial_human_bbox is accumulated as time flows
        ...
        ## accumulate 'initial_human_bbox'
        if self.initial_human_bbox is None:
            self.initial_human_bbox = seg2bbox(mask)
        else:
            self.initial_human_bbox = merge_bbox([self.initial_human_bbox, seg2bbox(mask)])
